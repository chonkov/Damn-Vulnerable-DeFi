...
describe("[Challenge] Climber", function () {
  ...
  it("Execution", async function () {
    ...
    const Hack = await ethers.getContractFactory("HackClimber");
    const hack = await Hack.connect(player).deploy();
    await hack
      .connect(player)
      .setup(vault.address, timelock.address, token.address, player.address);

    const targets = await hack.getTargets();
    const values = Array(targets.length).fill(0);
    const dataElements = await hack.getDataElements();
