...
describe("[Challenge] Free Rider", function () {
  ...
  before(async function () {
    ...
    uniswapPair = new ethers.ContractFactory(
      pairJson.abi,
      pairJson.bytecode,
      deployer
  ...
  it("Execution", async function () {
    ...
    const Hack = await ethers.getContractFactory("HackFreeRider");
    const hack = await Hack.connect(player).deploy(
      weth.address,
    ...
    await hack.connect(player).execute();
    expect(await ethers.provider.getBalance(hack.address)).to.be.closeTo(
      75n * 10n ** 18n,
      10n ** 17n
    );
  });
  ...
  after(async function () {
    ...
    expect(await ethers.provider.getBalance(marketplace.address)).to.be.eq(
      NFT_PRICE
    );
